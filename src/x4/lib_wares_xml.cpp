...
<ware id="module_gen_prod_hullparts_01" name="{20104,11301}" description="{20104,11302}" transport="container" volume="1" tags="module">
    ...
    <production time="862" amount="1" method="default" name="{20206,101}">
      <primary>
        <ware ware="claytronics" amount="614" />
        <ware ware="energycells" amount="1229" />
        <ware ware="hullparts" amount="2249" />                                                                                                                                                  </primary>
    </production>
    <research time="10">
      <research>
 ...
 **/
void to_module(const DOMNode& dom_node, RawMaterial& material) {
    // macro name
    // id
}
...
* Class methods
*******************************************************************************/
LibWaresXml::LibWaresXml(XmlParser&& xml_parser) : _xml_parser(std::move(xml_parser)) {}

LibWaresXml LibWaresXml::create(const string& file_path) {
    auto xml_parser = XmlParser::create(file_path);
    LibWaresXml wares_xml(std::move(xml_parser));
    return wares_xml;
}

unordered_map<string,EconomyWare> LibWaresXml::extract_economy_wares() const {
